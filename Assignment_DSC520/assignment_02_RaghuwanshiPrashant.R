...
person_df1 <- read.csv(file = "data/tidynomicon/person.csv", sep = ",", header=TRUE)
str(person_df1)
...
person_df2 <- read.csv(file = "data/tidynomicon/person.csv", sep = ",", header=TRUE, stringsAsFactors = FALSE)
str(person_df2)

## Read the file `data/scores.csv` to `scores_df`
## Display summary statistics using the `summary()` function
scores_df <- summary(read.csv(file = "data/tidynomicon/person.csv", sep = ",", header=TRUE))
scores_df
